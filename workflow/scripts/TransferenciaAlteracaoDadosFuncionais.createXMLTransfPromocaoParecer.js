function createXMLTransfPromocaoParecer(codReq){

	var xml = "";
		
	var dataHoje = ajusteData(buscarDataAtualSistema());
	xml += createNode("RhuReqPromocaoParecer");
	xml = createVRECTRANSFPROMOCAOPARECER(xml, codReq, dataHoje);
	xml += createNode("/RhuReqPromocaoParecer");
	
	return xml;
}

function createVRECTRANSFPROMOCAOPARECER(xml, codReq, dataHoje) {
	
	
		xml += createNode("VReqTransfPromocaoParecer");			
