function enviaAlteracao(){
	...
	try
	{
		...
		var xml = createXMLAlteracao();
				  
		log.info("#### NATHALIA XML: " + xml);
		log.info("#### NATHALIA contexto: " + contexto);
		log.info("#### NATHALIA userPass" + user + " - " + password);
		
		var result = authService.saveRecord("RhuReqPromocaoData", xml, contexto);
		
		log.info("--- NATHALIA RESULT WS: " + result.toString());
				
		if(isError(result.toString())){
			log.error('--- NATHALIA incluiRequisicao diz: Erro de negocio: ' + result);
