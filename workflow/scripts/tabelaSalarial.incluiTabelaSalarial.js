function incluiTabelaSalarial(tipoAcao){
	...
	try
	{
		...
		var xml = createXmlTabelaSalarial(tipoAcao);

		log.info("#### XML: " + xml);
		log.info("#### contexto: " + contexto);
		log.info("#### userPass" + user + " - " + password);
		
		if(xml != "<RhuTabSal></RhuTabSal>"){
		
			var result = authService.saveRecord("RhuTabSalData", xml, contexto);
