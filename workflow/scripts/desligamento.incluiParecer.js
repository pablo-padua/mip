function inclusaoParecerDeslig(codReq){
	...
	//try{
		...
		var codSistema = "V";
		var coligada = hAPI.getCardValue("codColigadaSolic");
		...
		var xml = createXmlRequisicaoParecerDeslig(codReq);
				  
		log.info("#### XML PARECER DESLIGAMENTO: " + xml);
		log.info("#### contexto: " + contexto);
		log.info("#### userPass: " + user + " - " + password);
		
		var result = authService.saveRecord("RhuReqDesligamentoParecerData", xml, contexto);
