function createXMLTransferencia(){

	var xml = "";		
	var dataHoje = ajusteData(buscarDataAtualSistema());
	
	xml += createNode("RhuReqTransferenciaData");
	xml = createVREQTRANSFERENCIA(xml, dataHoje);
	xml = createVREQTRANSFERENCIARATEIOCC(xml);
	xml += createNode("/RhuReqTransferenciaData");	
	
	return xml;
}

function createVREQTRANSFERENCIA(xml, dataHoje) {
		
	xml += createNode("VREQTRANSFERENCIA");			
...
function createVREQTRANSFERENCIARATEIOCC(xml) {
	
	var indexes = getIndexes('codCentroCustoProp');
	var iterator = indexes.iterator();
	while (iterator.hasNext()) {
		var index = iterator.next();
		xml += createNode("VREQTRANSFERENCIARATEIOCC");			
		xml += setNode("CODCOLREQUISICAO", hAPI.getCardValue("codColigadaOrigem"));	
		xml += setNode("IDREQ", "-1");		
