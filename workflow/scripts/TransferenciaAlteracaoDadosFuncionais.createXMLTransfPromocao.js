function createXMLTransfPromocao(){

	var xml = "";		
	var dataHoje = ajusteData(buscarDataAtualSistema());
	
	xml += createNode("RhuReqTransfPromocao");
	xml = createVREQTRANSFERENCIAPROMOCAO(xml, dataHoje);
	xml = createVREQTRANSFERENCIAPROMOCAORATEIOCC(xml);
	xml += createNode("/RhuReqTransfPromocao");	
	
	return xml;
}

function createVREQTRANSFERENCIAPROMOCAO(xml, dataHoje) {
		
	xml += createNode("VReqTransfPromocao");			
	xml += setNode("CODCOLREQUISICAO", hAPI.getCardValue("codColigadaOrigem"));	
	xml += setNode("IDREQ", "-1");	
	xml += setNode("JUSTIFICATIVA", "WRH07 - Autorização de Transferência - " + getValue("WKNumProces") + " - " + hAPI.getCardValue("justificativa"));	
	...
	xml += setNode("CODSECAOORG", hAPI.getCardValue("codSecaoOrigem"));	
	...
	xml += setNode("TRANSFERIRVAGA", "0");  // SEMPRE ZERO
	xml += createNode("/VReqTransfPromocao");
	
	return xml;
}

function createVREQTRANSFERENCIAPROMOCAORATEIOCC(xml) {
	
	var indexes = getIndexes('codCentroCustoProp');
	var iterator = indexes.iterator();
	while (iterator.hasNext()) {
		var index = iterator.next();
		xml += createNode("VReqTransfPromocaoRateioCC");			
		xml += setNode("CODCOLREQUISICAO", hAPI.getCardValue("codColigadaOrigem"));	
		xml += setNode("IDREQ", "-1");		
